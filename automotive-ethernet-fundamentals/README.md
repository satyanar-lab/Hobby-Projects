Excellent. README updates are the right next step — they keep the documentation honest while the code is still small and focused.
Let me set you up. Two READMEs to update:

Parent README (automotive-ethernet-fundamentals/README.md) — the resume-facing one, currently has the polished version we wrote earlier. We need to fix path references from udp-echo/ to someip/ and update file names.
someip/README.md — the per-project README, currently still says "UDP Echo." We need to update it to describe SOME/IP work specifically.

We'll do them one at a time. Parent first.

Step 1 — Verify where you are
cd ~/workspace/Hobby-Projects/automotive-ethernet-fundamentals
pwd
ls
Confirm:

pwd shows you in automotive-ethernet-fundamentals/
ls shows README.md and someip/ (and any other folders)

Paste me the output if anything looks off. If it looks right, we proceed.

Step 2 — Update the parent README
Open it:
vim README.md
I'll give you the updated version to use. This replaces the version we wrote earlier and fixes:

All paths reference someip/ instead of udp-echo/
New file names (someip_client.cpp, someip_server.cpp)
The structure section reflects the new directory layout
The "What's implemented" section stays honest — currently only SOME/IP REQUEST exists, nothing else yet
The "What's deliberately NOT implemented" section lists everything we haven't done yet, which is most things at the moment

Here's the new content:
markdown# Automotive Ethernet Fundamentals

Hand-built C++ implementations of automotive Ethernet protocols on Linux
using only POSIX sockets — no middleware, no vsomeip, no ara::com.

The goal is to understand the *bytes on the wire* at a level that's normally
hidden behind the AUTOSAR Adaptive stack. Every header field is constructed
and parsed by hand, every byte order conversion is explicit, and every design
